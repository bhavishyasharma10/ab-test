...
module.exports ={
  createTest: (name, config, options) => {
    ...
    const testName = name;
    const testLocalStorageKey = options.localStorageKey || 'ab-test';
    const maxRandomId = options.maxRandomId || 99999;
    ...
    const getGroup = (userId) => {
      ...
      if (userId === undefined || typeof userId !== 'string') {
        userId = Math.floor(Math.random() * maxRandomId) + 1;
        isRandomId = true;
      }

      if (storedTest && storedTest.name === testName && (isRandomId || storedTest.userId === userId)) {
